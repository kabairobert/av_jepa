...
metric_definitions = {
    ...
    "clean_flatness_ratio_a": (
        "Clean Flatness A",
        "Fraction of clean manifold A variance captured in its top PCA dimensions. Measures how well JEPA unrolls the latent space into a flat plane.",
        "(λ_1 + ... + λ_k) / sum(λ_i)"
    ),
    "clean_flatness_ratio_b": (
        "Clean Flatness B",
        "Fraction of clean manifold B variance captured in its top PCA dimensions.",
        "(λ_1 + ... + λ_k) / sum(λ_i)"
    ),
    "clean_orth_residual_a": (
        "Clean Orth Residual A",
        "Mean orthogonal distance of clean manifold A points from the top PCA plane. Measures unrolling/curvature error.",
        "Mean distance to PCA plane"
    ),
    "clean_orth_residual_b": (
        "Clean Orth Residual B",
        "Mean orthogonal distance of clean manifold B points from the top PCA plane.",
        "Mean distance to PCA plane"
    ),
    "flatness_ratio_a": (
        "Overall Flatness A",
    ...
    "pca_axis_align_a": (
        ...
        "Maximum cosine similarity between top PCA eigenvectors of A and standard coordinate axes. High value = axis-aligned.",
        "max_j |v_i^T e_j|"
    ),
    "pca_axis_align_b": (
        "PCA Axis-Alignment B",
        "Maximum cosine similarity between top PCA eigenvectors of B and standard coordinate axes.",
        "max_j |v_i^T e_j|"
    ),
...
# Build comprehensive LaTeX details for each metric
metric_details = {}
for m in ordered_metrics_list:
    display_name, desc, formula = metric_definitions.get(m, (m.replace("_", " ").title(), f"Evaluation metric: {m}", ""))
    ...
    <ul>
        <li>$U = [u_0, ..., u_{k-1}]^T \\in \\mathbb{R}^k$: True clean generative factors on shared manifold</li>
        <li>$X \\in \\mathbb{R}^d, Y \\in \\mathbb{R}^d$: Modality observed input ambient spaces (e.g. Audio, Video)</li>
        <li>$Z_A = f_A(X) \\in \\mathbb{R}^d, Z_B = f_B(Y) \\in \\mathbb{R}^d$: Transformed latent spaces</li>
    ...
    if m == "diagonality_ratio":
        ...
        $$D = \\frac{{\\sum_{{i=1}}^k C_{{i,i}}}}{{\\sum_{{i=1}}^d \\sum_{{j=1}}^d |C_{{i,j}}|}}$$
        </div>
        ...
        <h5>Mathematical Formulation:</h5>
        <div class="latex-block">
        $$\\text{{Score}} = \\sum_{{i=1}}^d \\rho_i \\cdot \\mathbb{{I}}\\left(i = \\text{{argmax}}_j \\ |\\text{{corr}}(Z_{{A,i}}, Z_{{B,j}})|\\right)$$
        </div>
        <h5>Explanation:</h5>
        ...
        <h5>Mathematical Formulation:</h5>
        <div class="latex-block">
        $$R_{{eff}} = \\exp\\left(-\\sum_{{i=1}}^d p_i \\ln p_i\\right) \\quad \\text{{where}} \\quad p_i = \\frac{{\\rho_i^2}}{{\\sum_{{j=1}}^d \\rho_j^2}}$$
        </div>
        <p>where $\\rho_i$ represents the correlation coefficient along canonical dimension $i$.</p>
        <h5>Explanation:</h5>
        <p>The effective rank of linear correlation pathways between adapters. Ideally matches the ground-truth $k$ (shared factors), indicating stable recovery of the shared generative manifold.</p>
        """
        ...
        <h5>Mathematical Formulation:</h5>
        <div class="latex-block">
        $$F = \\frac{{\\lambda_1 + ... + \\lambda_k}}{{\\sum_{{i=1}}^d \\lambda_i}}$$
        </div>
        <p>where $\\lambda_1 \\ge ... \\ge \\lambda_d$ are eigenvalues of the sample covariance matrix $\\Sigma = \\frac{{1}}{{N}} Z_{mod.lower()}^T Z_{mod.lower()}$ computed over {ptype}.</p>
        <h5>Explanation:</h5>
        ...
        <h5>Mathematical Formulation:</h5>
        <div class="latex-block">
        $$\\delta_{{orth}} = \\frac{{1}}{{N}} \\sum_{{k=1}}^N \\|z_{{mod.lower()}}^{{(k)}} - P_{{topK}}(z_{{mod.lower()}}^{{(k)}})\\|_2$$
        </div>
        <p>where $P_{{topK}} = V_k V_k^T$ is the orthogonal projection matrix onto the top-$k$ PCA unrolling plane.</p>
        <h5>Explanation:</h5>
        ...
        <h5>Mathematical Formulation:</h5>
        <div class="latex-block">
        $$\\mathbb{{E}}[\\|z\\|_2] = \\frac{{1}}{{N}} \\sum_{{k=1}}^N \\|z^{{(k)}}\\|_2$$
        </div>
        <h5>Explanation:</h5>
...
print("Querying WandB for B09 runs...")
runs = api.runs(f"{entity}/{project}", filters={"tags": {"$in": [batch_id]}})

# Group runs by configuration name (e.g. B09_D0_N1P11)
cfg_to_runs = {}
for r in runs:
    cfg_tag = next((tag for tag in r.tags if tag.startswith("B09_")), None)
    if cfg_tag:
...
for cfg_file in all_cfg_files:
    ...
    # Extraction for index sorting
    match = re.search(r"B09_(D\d)_N(\d)P(\d)(\d)", cfg_name)
    if not match:
        continue
    d_code, n_idx, p1_idx, p2_idx = match.groups()
    ...
    with open(cfg_file, "r") as yf:
        ...
        d_out = c['data']['d_out']
        k_shared = c['data']['k_shared']
        m_unique = c['data']['m_unique']
        dataset_str = f"D:{d_out}/k:{k_shared}/m:{m_unique}"
        
        dataset_desc = dataset_display_map.get(d_code, f"Dataset {d_code}")
    ...
    for alias in ordered_metrics_list:
        ...
        # If float, round to 4 decimal places
        if isinstance(val, (int, float)):
            metrics_data[alias] = round(float(val), 4)
        else:
            metrics_data[alias] = None  # Use Python None, JSON serializer will correctly convert to null
...
# HTML generation with B07 light theme
html_content = f"""<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>B09 nd-kf-mlp Sweep Dashboard</title>
    ...
    <style>
        ...
        /* Dictionary panel matching B07 style (closed by default) */
        .dictionary-box {{
            background: #f6f8fa;
            border: 1px solid #d0d7de;
            border-radius: 6px;
        ...
        /* Table Style matching B07 exactly */
        table {{
            width: 100%;
        ...
        /* Visualization Embed Column matching B07 exact widths */
        .embed-cell {{
            padding: 0;
            width: 80%;
        }}
...
<body>
    <h1>B09 nd-kf-mlp Sweep <span>Dashboard</span></h1>
...
{dict_cards_str}
    ...
    <div class="filters">
        <div>
            <div class="filter-group">
                <label for="filter-dataset">Dataset [D]:</label>
                <select id="filter-dataset" onchange="applyFilters()">
                    <option value="all" selected>All</option>
                    <option value="D0: 3D (k=2, m=1)">[D0] 3D (k=2, m=1)</option>
                    <option value="D1: 10D (k=2, m=8)">[D1] 10D (k=2, m=8)</option>
                    <option value="D2: 10D (k=5, m=5)">[D2] 10D (k=5, m=5)</option>
                    <option value="D3: 20D (k=5, m=15)">[D3] 20D (k=5, m=15)</option>
                </select>
            </div>
            <div class="filter-group">
                ...
                <select id="filter-noise" onchange="applyFilters()">
                    <option value="all" selected>All</option>
                    <option value="Asym 5% (asym05_ext0)">[1] Asym 5%</option>
                    <option value="Asym 15% (asym15_ext0)">[2] Asym 15%</option>
                    <option value="Ext 30% (asym0_ext30)">[3] Ext 30%</option>
                </select>
            </div>
            <div class="filter-group">
    ...
    <div class="footer-action">
        <span>B09 Sweep • High-Dimensional MLP Manifold Study</span>
    ...
    <script>
        ...
        function renderCheckboxes() {{
            ...
            const helperColumns = [
                {{ id: "dataset_col", label: "Dataset Column" }},
        ...
        function renderTable() {{
            ...
            thConfig.textContent = "Config" + getHeaderIcon('config');
            thConfig.className = "col-config";
            thConfig.onclick = () => handleSort('config');
            headers.appendChild(thConfig);

            // Separate parameters cells
            if (columnState.dataset_col) {{
                const th = document.createElement("th");
                th.textContent = "Dataset" + getHeaderIcon('dataset_col');
                th.className = "col-param";
                th.onclick = () => handleSort('dataset_col');
        ...
        function applyFilters() {{
            const fDataset = document.getElementById("filter-dataset").value;
            ...
            let filteredRows = runData.filter(row => {{
                const matchDataset = (fDataset === 'all' || row.dataset === fDataset);
                ...
                return matchDataset && matchNoise && matchPrior && matchPred;
            }});
            ...
            if (currentSortCol && currentSortDir !== 'none') {{
                filteredRows.sort((a, b) => {{
                    ...
                    if (currentSortCol === 'config') {{
                        valA = a.config;
                        valB = b.config;
                    }} else if (currentSortCol === 'dataset_col') {{
                        valA = a.dataset;
                        valB = b.dataset;
        ...
        function createRow(row) {{
            ...
            tdConfig.innerHTML = `${{row.config}}<span class="config-label">D:${{row.d_code}} N:${{row.n_idx}} P1:${{row.p1_idx}} P2:${{row.p2_idx}}</span>
            <span style="font-size: 11px; color: #57606a; display: block; margin-top: 5px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; font-weight: normal; line-height: 1.45; white-space: nowrap;">
                <strong>Data:</strong> ${{row.dataset_str}}<br>
            ...
            allMetrics.forEach(m => {{
                if (columnState[m]) {{
                    ...
                    if (val === null || val === undefined) {{
                        ...
                        // Apply subtle cell background colors matching GitHub light themes
                        ...
                        if (ratio >= 0.5) {{
                            const intensity = (ratio - 0.5) * 2; // [0, 1]
                            td.style.backgroundColor = `rgba(46, 160, 67, ${{intensity * 0.16}})`; // soft green
                        }} else {{
                            const intensity = (0.5 - ratio) * 2; // [0, 1]
                            td.style.backgroundColor = `rgba(248, 81, 73, ${{intensity * 0.16}})`; // soft red
                        }}
                    }}
                    tr.appendChild(td);
                }}
            ...
            // Visualization Inline Cell (Lazy iframe loading)
            if (columnState.visualization) {{
                const td = document.createElement("td");
        ...
        function exportCSV() {{
            ...
            const csvHeaders = ["Config", "Dataset", "Noise", "Prior", "Predictor", "State"];
            ...
            runData.forEach(row => {{
                ...
                if (matchDataset && matchNoise && matchPrior && matchPred) {{
                    const rowData = [
                        row.config,
                        row.dataset_str,
                        row.noise_str,
            ...
            link.setAttribute("download", `B09_Sweep_metrics_${{fDataset.replace(/\\s+/g, '_')}}.csv`);
        ...
        function openModal(metricId) {{
            ...
            if (details) {{
                ...
                // Trigger MathJax typesetting on the modal content
