...
cfg_dir = SCRIPT_DIR.parent.parent.parent / "cfgs" / "B16c_capacity_scaling_fixed"
...
for m in ordered_metrics_list:
    ...
    <ul>
        ...
        <li>$Z_A = f_A(X) \\in \\mathbb{R}^{20}, Z_B = f_B(Y) \\in \\mathbb{R}^{20}$: Transformed latent spaces</li>
    ...
    if m == "diagonality_ratio":
        ...
        $$D = \\frac{{\\sum_{{i=1}}^2 C_{{i,i}}}}{{\\sum_{{i=1}}^{{20}} \\sum_{{j=1}}^{{20}} |C_{{i,j}}|}}$$
        </div>
        ...
        <h5>Mathematical Formulation:</h5>
        <div class="latex-block">
        $$\\text{{Score}} = \\sum_{{i=1}}^{{20}} \\rho_i \\cdot \\mathbb{{I}}\\left(i = \\text{{argmax}}_j \\ |\\text{{corr}}(Z_{{A,i}}, Z_{{B,j}})|\\right)$$
        </div>
        <h5>Explanation:</h5>
        ...
        <h5>Mathematical Formulation:</h5>
        <div class="latex-block">
        $$R_{{eff}} = \\exp\\left(-\\sum_{{i=1}}^{{20}} p_i \\ln p_i\\right) \\quad \\text{{where}} \\quad p_i = \\frac{{\\rho_i^2}}{{\\sum_{{j=1}}^{{20}} \\rho_j^2}}$$
        </div>
        ...
        <h5>Mathematical Formulation:</h5>
        <div class="latex-block">
        $$F = \\frac{{\\lambda_1 + \\lambda_2}}{{\\sum_{{i=1}}^{{20}} \\lambda_i}}$$
        </div>
        <p>where $\\lambda_1 \\ge \\lambda_2$ are top eigenvalues of the sample covariance matrix computed over {ptype}.</p>
        <h5>Explanation:</h5>
        ...
        <p>where $P_{{top2}}$ is the orthogonal projection matrix onto the top-2 PCA unrolling plane.</p>
        <h5>Explanation:</h5>
...
project = "eb_jepa"
batch_id = "B16c_capacity_scaling_fixed"

print("Querying WandB for B16c runs...")
runs = api.runs(f"{entity}/{project}", filters={"tags": {"$in": [batch_id]}})

# Group runs by configuration name
cfg_to_runs = {}
for r in runs:
    cfg_tag = next((tag for tag in r.tags if tag.startswith("B16c_")), None)
    if cfg_tag:
...
def get_sort_key(cfg_path):
    name = cfg_path.stem
    match = re.search(r"B16c_(\d+)D_S(\d+)", name)
    if match:
...
all_cfg_files = sorted(list(cfg_dir.glob("B16c_*.yaml")), key=get_sort_key)

for cfg_file in all_cfg_files:
    cfg_name = cfg_file.stem
    
    # Parser regex optimized for B16c structure
    npp_match = re.search(r"B16c_(\d+)D_S(\d+)", cfg_name)
    if not npp_match:
        continue
    scale, stages = npp_match.groups()
    n_idx, p1_idx, p2_idx = "1", "2", "1"

    # Read parameters from yaml configuration
    with open(cfg_file, "r") as yf:
        c = yaml.safe_load(yf)
        asym_rate_a = c['data'].get('asymmetric_noise_rate_a', c['data'].get('asym_corrupt_rate_a', 0.0))
        ext_noise_ratio = c['data'].get('external_noise_ratio', 0.1)
        noise_str = f"Asy:{asym_rate_a}/Ext:{ext_noise_ratio}"
        ...
        noise_desc = "Ext 10%"
        prior_desc = "L2 Prior"
        pred_desc = "L1 Predictor"
        
        # In B16c, the dataset is 3d-3f-2c-mlp, which always has:
        k_shared = 2
        m_unique = 1
...
<head>
    <meta charset="UTF-8">
    <title>B16c Scaling Disentanglement Dashboard</title>
...
<body>
    <h1>B16c Scaling Disentanglement Sweep <span>Dashboard</span></h1>
...
{dict_cards_str}
    ...
    <div class="footer-action">
        <span>B16c Scaling Sweep • Non-Volume-Preserving Flow study</span>
    ...
    <script>
        ...
        function renderTable() {{
            ...
            thConfig.textContent = "Config (D S K M)" + getHeaderIcon('config');
            ...
            if (columnState.stages_col) {{
                const th = document.createElement("th");
                th.textContent = "Stages" + getHeaderIcon('stages_col');
                th.className = "col-param";
                th.onclick = () => handleSort('stages_col');
                headers.appendChild(th);
            }}
        ...
        function applyFilters() {{
            ...
            const fStages = document.getElementById("filter-stages").value;
        ...
        function createRow(row) {{
            ...
            tdConfig.innerHTML = `${{row.config}}<span class="config-label">D: ${{row.scale}} | S: ${{row.stages}} | K: ${{row.k_shared}} | M: ${{row.m_unique}}</span>
            ...
            if (columnState.stages_col) {{
                const td = document.createElement("td");
                td.className = "col-param";
                td.textContent = row.stages + ' Stages';
                tr.appendChild(td);
            }}
        ...
        function exportCSV() {{
            ...
            const csvHeaders = ["Config", "Scale", "Stages", "State"];
            ...
            link.setAttribute("download", `B16c_Sweep_metrics_${{fScale.replace(/\\s+/g, '_')}}.csv`);
