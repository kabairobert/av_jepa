...
runs = api.runs(f"{entity}/{project}", filters={"tags": {"$in": ["B07_loss_noise_interaction"]}})
...
for html_file in checkpoint_root.glob("**/interactive_3d_4way_html_*.html"):
    # Path looks like: .../wandb/run-TIMESTAMP-RUNID/files/media/html/...
    ...
    if run_id:
        if run_id not in run_id_to_latest_html:
            ...
            # Keep the one with higher step number (if possible to parse) or just latest mtime
...
cfg_to_run = {}
for run in runs:
    cfg_tag = next((tag for tag in run.tags if tag.startswith("B07_NPP")), None)
    if cfg_tag:
        cfg_to_run[cfg_tag] = {
            "id": run.id,
            "state": run.state,
...
status_file = Path("/gpfs/home3/rkabai/github/eb_jepa_private/multimodal_experiments/ssl_dual_alignment/hyp_testing/batches/B07_loss_noise_interaction/STATUS.md")
status_dir = status_file.parent

with open(status_file, "w") as f:
    f.write("# B07 Sweep Status\n\n")
    ...
    for cfg_file in all_cfg_files:
        ...
        with open(cfg_file, "r") as y:
            ...
            if c['data'].get('manifold_noise_a', 0.02) > 0.02:
                noise += f"/Mnf:{c['data']['manifold_noise_a']}"
