"""B12 status update — thin wrapper over shared update_status_lib."""
import sys
...
def _b12_params(cfg: dict) -> str:
    return str(cfg['data']['num_samples'])


generate_status_md(
    batch_tag="B12_data_scaling",
    batch_id_str="B12",
    cfg_files=sorted(cfg_dir.glob("B12_*.yaml")),
    status_file=SCRIPT_DIR / "STATUS.md",
    checkpoint_root=ROOT_DIR / "checkpoints" / "sslda",
    cfg_to_params_fn=_b12_params,
)
