...
cfg_dir = SCRIPT_DIR.parent.parent.parent / "cfgs" / "B16_capacity_scaling_fixed"
...
project = "eb_jepa"
batch_id = "B16_capacity_scaling_fixed"

print("Querying WandB for B16 runs...")
runs = api.runs(f"{entity}/{project}", filters={"tags": {"$in": [batch_id]}})

# Group runs by configuration name
cfg_to_runs = {}
for r in runs:
    cfg_tag = next((tag for tag in r.tags if tag.startswith("B16_")), None)
    if cfg_tag:
...
def get_sort_key(cfg_path):
    name = cfg_path.stem
    match = re.search(r"B16_(\d+)D_S(\d+)", name)
    if match:
...
all_cfg_files = sorted(list(cfg_dir.glob("B16_*.yaml")), key=get_sort_key)

for cfg_file in all_cfg_files:
    cfg_name = cfg_file.stem
    
    # Parser regex optimized for B16 structure
    npp_match = re.search(r"B16_(\d+)D_S(\d+)", cfg_name)
    if not npp_match:
        continue
...
<head>
    <meta charset="UTF-8">
    <title>B16 Scaling Disentanglement Dashboard</title>
...
<body>
    <h1>B16 Scaling Disentanglement Sweep <span>Dashboard</span></h1>
...
{dict_cards_str}
    ...
    <div class="footer-action">
        <span>B16 Scaling Sweep • Non-Volume-Preserving Flow study</span>
    ...
    <script>
        ...
        function renderTable() {{
            ...
            thConfig.textContent = "Config (N S)" + getHeaderIcon('config');
        ...
        function exportCSV() {{
            ...
            link.setAttribute("download", `B16_Sweep_metrics_${{fScale.replace(/\\s+/g, '_')}}.csv`);
