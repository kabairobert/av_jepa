...
print("Querying WandB for B08 runs...")
runs = api.runs(f"{entity}/{project}", filters={"tags": {"$in": [batch_id]}})

# Group runs by configuration name (e.g. B08_NPP111)
cfg_to_runs = {}
for r in runs:
    cfg_tag = next((tag for tag in r.tags if tag.startswith("B08_NPP")), None)
    if cfg_tag:
...
for cfg_file in all_cfg_files:
    ...
    # NPP extraction for index sorting
    npp_match = re.search(r"B08_NPP(\d)(\d)(\d)", cfg_name)
    if not npp_match:
        continue
...
<head>
    <meta charset="UTF-8">
    <title>B08 Volumetric Sweep Dashboard</title>
...
<body>
    <h1>B08 Volumetric Sweep <span>Dashboard</span></h1>
...
{dict_cards_str}
    ...
    <div class="footer-action">
        <span>B08 Sweep • Volumetric Dual-Alignment Study</span>
        <button class="csv-btn" onclick="exportCSV()">📥 Download Filtered CSV</button>
    </div>

    <script>
        // CRITICAL BUGFIX: Dump variables via json.dumps to yield valid JavaScript
        ...
        function createRow(row) {{
            ...
            // Config Cell (Includes descriptive Parameters directly underneath in separate rows)
        ...
        function exportCSV() {{
            ...
            link.setAttribute("download", `B08_Sweep_metrics_${{fNoise.replace(/\\s+/g, '_')}}.csv`);
