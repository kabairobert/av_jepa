...
runs = api.runs(f"{entity}/{project}", filters={"tags": {"$in": ["B08_volumetric_alignment"]}})
...
cfg_to_run = {}
for run in runs:
    cfg_tag = next((tag for tag in run.tags if tag.startswith("B08_NPP")), None)
    if cfg_tag:
        cfg_to_run[cfg_tag] = {
            "id": run.id,
            "state": run.state,
