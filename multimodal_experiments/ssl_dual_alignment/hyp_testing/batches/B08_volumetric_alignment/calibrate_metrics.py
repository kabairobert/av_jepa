#!/usr/bin/env python3
"""
B08 Metric Calibration Script  (v2 — AV-JEPA goal-aligned)
=============================================================
Pulls all B08 wandb runs, ranks (prior, pred) combos by individual metrics and
composite scores, then checks whether the metric-based ranking reproduces the
visual ground-truth from the 3D scatterplots.

Visual ground-truth (from human inspection of B08 plots):
...
Output: prints tables to stdout + saves B08_calibration.csv
"""
...
OUT_CSV      = SCRIPT_DIR / "B08_calibration.csv"
...
for r in runs:
    cfg_tag = next((t for t in r.tags if t.startswith("B08_NPP")), None)
    if cfg_tag:
...
# ── Build rows ─────────────────────────────────────────────────────────────────
rows = []
for cfg_file in sorted(cfg_dir.glob("B08_NPP*.yaml")):
    cfg_name = cfg_file.stem
    m = re.search(r"B08_NPP(\d)(\d)(\d)", cfg_name)
    if not m:
        continue
    n_idx, p1_idx, p2_idx = m.groups()
...
# All sub-scores min-max normalised (0=worst, 1=best within B08 range).
