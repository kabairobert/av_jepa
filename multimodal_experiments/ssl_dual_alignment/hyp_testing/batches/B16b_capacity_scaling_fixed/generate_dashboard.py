...
project = "eb_jepa"
batch_id = "B16b_capacity_scaling_fixed"

print("Querying WandB for B16b runs...")
runs = api.runs(f"{entity}/{project}", filters={"tags": {"$in": [batch_id]}})

# Group runs by configuration name
cfg_to_runs = {}
for r in runs:
    cfg_tag = next((tag for tag in r.tags if tag.startswith("B16b_")), None)
    if cfg_tag:
...
def get_sort_key(cfg_path):
    name = cfg_path.stem
    match = re.search(r"B16b_(\d+)D_S(\d+)", name)
    if match:
...
all_cfg_files = sorted(list(cfg_dir.glob("B16b_*.yaml")), key=get_sort_key)

for cfg_file in all_cfg_files:
    cfg_name = cfg_file.stem
    
    # Parser regex optimized for B16b structure
    npp_match = re.search(r"B16b_(\d+)D_S(\d+)", cfg_name)
    if not npp_match:
        continue
    ...
    with open(cfg_file, "r") as yf:
        c = yaml.safe_load(yf)
        asym_rate_a = c['data'].get('asym_corrupt_rate_a', c['data'].get('asymmetric_noise_rate_a', 0.0))
        ...
        m_unique = int(c['data'].get('m_unique', 0))
...
<head>
    <meta charset="UTF-8">
    <title>B16b Scaling Disentanglement Dashboard</title>
...
<body>
    <h1>B16b Scaling Disentanglement Sweep <span>Dashboard</span></h1>
...
{dict_cards_str}
    ...
    <div class="footer-action">
        <span>B16b Scaling Sweep • Non-Volume-Preserving Flow study</span>
    ...
    <script>
        ...
        function createRow(row) {{
            ...
            tdConfig.innerHTML = `${{row.config}}<span class="config-label">D: ${{row.scale}} | S: ${{row.stages}} | K: 3 | M: ${{row.m_unique}}</span>
        ...
        function exportCSV() {{
            ...
            link.setAttribute("download", `B16b_Sweep_metrics_${{fScale.replace(/\\s+/g, '_')}}.csv`);
