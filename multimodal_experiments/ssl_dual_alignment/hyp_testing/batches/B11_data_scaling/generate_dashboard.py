...
project = "eb_jepa"
batch_id = "B11_data_scaling"

print("Querying WandB for B10 runs...")
runs = api.runs(f"{entity}/{project}", filters={"tags": {"$in": [batch_id]}})

# Group runs by configuration name (e.g. B10_NPP111)
cfg_to_runs = {}
for r in runs:
...
for cfg_file in all_cfg_files:
    ...
    npp_match = re.search(r"B11_(\d+x)_([RM])(\d+)_N1P21", cfg_name)
    if not npp_match:
        continue
...
<head>
    <meta charset="UTF-8">
    <title>B11 Data Scaling Dashboard</title>
...
{dict_cards_str}
    ...
    <div class="footer-action">
        <span>B10 Sweep • Volumetric Dual-Alignment Study</span>
