        ...
        # Note: RNN predictors (is_rnn=True) are a special case with ctxt_window_time=1
            ...
            for i in range(nsteps):
                ...
                if compute_loss:
                    ...
                    # Guard against empty pred_step (misaligned unroll config)
                    if hasattr(pred_step, "shape") and len(pred_step.shape) >= 3 and pred_step.shape[2] == 0:
                        raise RuntimeError(
                            f"Empty pred_step at i={i}; pred_step.shape={tuple(pred_step.shape)}; "
                            f"nsteps={nsteps}; effective_ctxt_window={effective_ctxt_window}; state_T={state.shape[2]}. "
                            "Check ctxt_window_time vs predictor.context_length."
                        )
